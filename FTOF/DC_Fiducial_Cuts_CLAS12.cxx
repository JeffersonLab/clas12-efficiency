...
/// required avriablöes and functions:

// // select the field setting in a global variable:
// bool GetMyfid(Double_t part_DC_c1x,Double_t part_DC_c1y,Double_t part_DC_c1z,
//   Double_t part_DC_c2x,Double_t part_DC_c2y,Double_t part_DC_c2z,
//   Double_t part_DC_c3x,Double_t part_DC_c3y,Double_t part_DC_c3z,Int_t pid){
  ...
  /// from REC::Partcile
  // Define functions in CLAS12ROOT



  /// from REC::Traj
  // // x,y and z positions for each region
  ...
  // }
  // else if(p->traj(DC,18)->getDetector() == 6 && p->traj(DC,18)->getLayer() == 18){
  //   part_DC_c2x = p->traj(DC,18)->getX();
  //   part_DC_c2y = p->traj(DC,18)->getY();
  //   part_DC_c2z = p->traj(DC,18)->getZ();
  // }
  // else if(p->traj(DC,36)->getDetector() == 6 && p->traj(DC,36)->getLayer() == 36){
  ...
  int determineSector(Double_t part_DC_c2x, Double_t part_DC_c2y,Double_t part_DC_c2z){
    // Value of pi
    Double_t Pival = 3.141592653589793238462643383279502884;

    double phi = 180 / Pival * atan2(part_DC_c2y / sqrt(pow(part_DC_c2x, 2) + pow(part_DC_c2y, 2) + pow(part_DC_c2z, 2)), part_DC_c2x / sqrt(pow(part_DC_c2x, 2)
    + pow(part_DC_c2y, 2) + pow(part_DC_c2z, 2)));
  ...
  bool DC_fiducial_cut_theta_phi(Double_t part_DC_c1x,Double_t part_DC_c1y,Double_t part_DC_c1z,
    Double_t part_DC_c2x,Double_t part_DC_c2y,Double_t part_DC_c2z,
    Double_t part_DC_c3x,Double_t part_DC_c3y,Double_t part_DC_c3z,Int_t part_pid,int part_DC_sector,int region){
        inbending=true;
        outbending=false;
      Double_t Pival = 3.141592653589793238462643383279502884;
      ...
      const double maxparams_in[6][6][3][4] = {
        ...
        const double minparams_in[6][6][3][4] = {
          ...
          //fitted values outbending (not used and not updated)
          ...
          const double maxparams_out[6][6][3][4] = {
            ...
            const double minparams_out[6][6][3][4] = {
              ...
              switch (region)
              {
                ...
                phi_DCr_raw = 180 / Pival * atan2(part_DC_c1y / sqrt(pow(part_DC_c1x,2) + pow(part_DC_c1y,2) + pow(part_DC_c1z,2)),
                ...
                phi_DCr_raw = 180 / Pival * atan2(part_DC_c2y / sqrt(pow(part_DC_c2x,2) + pow(part_DC_c2y,2) + pow(part_DC_c2z,2)),
                ...
                phi_DCr_raw = 180 / Pival * atan2(part_DC_c3y / sqrt(pow(part_DC_c3x,2) + pow(part_DC_c3y,2) + pow(part_DC_c3z,2)),
              ...
              if (part_DC_sector == 1) phi_DCr = phi_DCr_raw;
              if (part_DC_sector == 2) phi_DCr = phi_DCr_raw - 60;
              if (part_DC_sector == 3) phi_DCr = phi_DCr_raw - 120;
              if (part_DC_sector == 4 && phi_DCr_raw > 0) phi_DCr = phi_DCr_raw - 180;
              if (part_DC_sector == 4 && phi_DCr_raw < 0) phi_DCr = phi_DCr_raw + 180;
              if (part_DC_sector == 5) phi_DCr = phi_DCr_raw + 120;
              if (part_DC_sector == 6) phi_DCr = phi_DCr_raw + 60;

              int pid = 0;
              // Positive particles asssigned to pi^+, negative to pi^-
              // if(p->par()->getCharge()>0) pid = 2;
              // else if(p->par()->getCharge()<0) pid = 3;
              ...
              double calc_phi_min = minparams[pid][part_DC_sector - 1][region][0] + minparams[pid][part_DC_sector - 1][region][1] * std::log(theta_DCr)
              + minparams[pid][part_DC_sector - 1][region][2] * theta_DCr + minparams[pid][part_DC_sector - 1][region][3] * theta_DCr * theta_DCr;

              double calc_phi_max = maxparams[pid][part_DC_sector - 1][region][0] + maxparams[pid][part_DC_sector - 1][region][1] * std::log(theta_DCr)
              + maxparams[pid][part_DC_sector - 1][region][2] * theta_DCr + maxparams[pid][part_DC_sector - 1][region][3] * theta_DCr * theta_DCr;
            ...
            bool DC_fiducial_cut_XY(Double_t part_DC_c1x,Double_t part_DC_c1y,Double_t part_DC_c1z,
              Double_t part_DC_c2x,Double_t part_DC_c2y,Double_t part_DC_c2z,
              Double_t part_DC_c3x,Double_t part_DC_c3y,Double_t part_DC_c3z,Int_t part_pid,int part_DC_sector,int region)
              {
                  outbending=true;
                  ...
                  const double minparams_in[6][6][3][2] = {
                    ...
                    const double maxparams_out[6][6][3][2] = {
                      ...
                      const double minparams_out[6][6][3][2] = {
                        ...
                        double calc_min = minparams[pid][part_DC_sector - 1][region][0] + minparams[pid][part_DC_sector - 1][region][1] * X;
                        double calc_max = maxparams[pid][part_DC_sector - 1][region][0] + maxparams[pid][part_DC_sector - 1][region][1] * X;
